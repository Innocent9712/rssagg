...
const getPostsOfFeedUserFollows = `-- name: GetPostsOfFeedUserFollows :many
...
func (q *Queries) GetPostsOfFeedUserFollows(ctx context.Context, userID uuid.UUID) ([]Post, error) {
	rows, err := q.db.QueryContext(ctx, getPostsOfFeedUserFollows, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
